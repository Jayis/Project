...
public class TimbreDistributionExtractor implements AudioFeatureExtractor {
	...
	public TimbreDistributionExtractor(String featureType){
        this(featureType,"");
        if(featureType.equals("MFCC")){
        	this.mfcc = new MFCC(AudioPreProcessor.DEFAULT_SAMPLE_RATE);
        }
        else if(featureType.equals("SSD")){
			this.mfcc = new SSD(AudioPreProcessor.DEFAULT_SAMPLE_RATE);
		}
    }
	public TimbreDistributionExtractor(String featureType,String filename){
        try{
		featureType=featureType.toUpperCase();
		if(featureType.equals("MFCC")){
            File file = new File(filename);
            AudioInputStream ais = AudioSystem.getAudioInputStream(file);
            double[] meanbuf = new double[512];
            AudioPreProcessor prep =new AudioPreProcessor(ais);
            int meanRead = prep.append(meanbuf,0,512);
            int count=0;
            double mean=0;

            while(meanRead == 512){
                mean+= new Mean().evaluate(meanbuf);
                count+=1;
                meanRead = prep.append(meanbuf,0,512);
            }

            mean/=count;
			this.mfcc = new MFCC(AudioPreProcessor.DEFAULT_SAMPLE_RATE,mean);
        }
		else if(featureType.equals("SSD")){
			this.mfcc = new SSD(AudioPreProcessor.DEFAULT_SAMPLE_RATE);
		}
...
*/
	...
	public Attribute calculate(Object input )
		...
		Vector mfccCoefficients = mfcc.process(preProcessor);//calls mfcc.process
        
		double[] points =  (double[]) mfccCoefficients.get(0);
