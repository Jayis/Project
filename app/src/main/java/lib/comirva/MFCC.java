 ...
 * @version 1.0
 */
public class MFCC implements Feature
{
  //general fields
  ...
  public MFCC(float sampleRate) throws IllegalArgumentException{
	  this(sampleRate,1024, 13, false, 20.0, 8000.0, 20,0);
  }
  public MFCC(float sampleRate,double mean) throws IllegalArgumentException
  {
    this(sampleRate,1024, 13, false, 20.0, 8000.0, 20,mean);
  }
   ...
   * @throws IllegalArgumentException raised if mehtod contract is violated
   */
  public MFCC(float sampleRate, int windowSize, int numberCoefficients, boolean useFirstCoefficient,double mean) throws IllegalArgumentException
  {
    this(sampleRate, windowSize, numberCoefficients, useFirstCoefficient, 24.0, 8000.0, 20,mean);
  }
   ...
   * @throws IllegalArgumentException raised if mehtod contract is violated
   */
  public MFCC(float sampleRate, int windowSize, int numberCoefficients, boolean useFirstCoefficient, double minFreq, double maxFreq, int numberFilters, double mean) throws IllegalArgumentException
  {
 ...
 private Matrix getMelFilterBanks()
  {
    ...
    {
      double[] filter = new double[windowSize];
    ...
    return new Matrix(matrix, numberFilters, windowSize);
  }
  ...
  private double getMelFilterWeight(int filterBank, double freq, double[] boundaries)
  {
    ...
    double height = 1.0d;//2.0d/(end - start);
  ...
  private Matrix getDCTMatrix()
  {
    ...
    double w2 = Math.sqrt(2.0/numberFilters);//*(Math.sqrt(2.0)/2.0);
  ...
  public Vector process(AudioPreProcessor in) throws IllegalArgumentException, IOException
  {
    ...
    //System.out.println(in.getSampleRate()+"in");
    //System.out.println(sampleRate+"no");
    ...
    int count=0;
    double meantest=0;
    int maxcount = Integer.valueOf(Math.round(30*sampleRate/hopSize));
    //process all other frames
    while (samplesRead == hopSize && count <maxcount)
    {
      last = inputData[hopSize-1];
      ...
      meantest += new Mean().evaluate(inputData);
      count += 1;
      //process the current window
      mfcc.add(processWindow(inputData, 0,last));
    }
    meantest/=count;
...
*/
   ...
   * @throws IllegalArgumentException raised if mehtod contract is violated
   */
  public double[] processWindow(double[] window, int start,double prev) throws IllegalArgumentException
  {
    double a = 0.95;
    double ham=0.46;
    double eps=3e-16;
    ...
    //just copy to buffer pre emphasize
    for (int j = 1; j < windowSize; j++)
      buffer[j] = (window[j]-mean)-a*(window[j-1]-mean);

    buffer[0] = window[0]-a*prev;

    //hamming window
    /*
    double k = 2*Math.PI/(windowSize-1);
    for (int j=0;j<windowSize;j++){
        buffer[j] *= (1-ham) - ham* Math.cos(k*j);
    }
*/
    //perform power fft
    ...
    double logEnergy=10*Math.log(energy) / Math.log(10);
    double[] coeffs=x.getColumnPackedCopy();

    double[] result=new double[coeffs.length+1];
    result[coeffs.length]=logEnergy;
